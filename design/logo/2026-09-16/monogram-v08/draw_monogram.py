...
# Reference a: a full upper bowl feeding a long oblique outer contour,
# then a soft curved taper at the base. The inner edge stays at x=152.
# No tilted leg or abrupt chamfer corner.
old_segment='C169 84 188 118 188 168 V318 H152 V174'
new_segment='C174 84 208 112 208 160 C208 184 196 211 185 239 L170 282 C165 298 160 311 152 318 H152 V174'
assert original_h.count(old_segment)==1
monogram[1]['d']=original_h.replace(old_segment,new_segment)
assert monogram[1]['d'].replace(new_segment,old_segment)==original_h
assert monogram[1]['d'].split('H152 V174',1)[1]==original_h.split('H152 V174',1)[1]

for name,color in [('ivory',IVORY),('gold',GOLD),('ink',INK)]:
    (OUT/f'eh-contour-{name}.svg').write_text(svg(place(monogram,0,0,1,color),421,405,'ElevenHouse — eh со скосом нижней правой части h'))
